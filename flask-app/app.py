...
def get_invoice():
    ...
    logging.warning(f"Service name = {service_name}")
    ...
    if upstream_service is not None:
        ...
        logging.warning(f"Found services = {found_services}")
...
def register(client):
    ...
    while True:
        try:
            client.agent.service.register(service_name, address=service_name, port=int(port), check=check_http, tags=[service_name])
            break
        except (ConnectionError, consul.ConsulException) as e:
            logging.error(f"Consul host is down, reconnecting after 500ms...: reason = {e}")
            time.sleep(5)


if __name__ == "__main__":
    port = os.environ.get("SERVICE_PORT")
    logging.warning(f"Listening for port {port}")
    cons = consul.Consul(host="consul", port=8500)
    logging.warning(f"Consul client = {cons}")
    register(cons)
